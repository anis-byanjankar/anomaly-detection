...
def scores(x, forest, n):
    '''Input: x: features data frame for individual tile
    forest: trained forest
    n: subsample size
    Output: evaluation score for individual tile
    '''
    score_collect = []
...
def anomaly_frames(frames, t, n, tile):
    ...
    df = get_features(frames, topleft, bottomright)
    test = iForest(df, t, n).populate()
    return scores(df, test, n)
